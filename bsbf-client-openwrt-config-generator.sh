...
usage() {
	echo "Usage: $0 --server-ipv4 <ADDR> --server-port <PORT> --uuid <UUID> [--no-luci --dongle-modem --mbim-modem --usb-adapters-and-android-tethering --ios-tethering --mikrotik-tools --diag-tools --perf-test]"
	exit 1
}
...
while [ $# -gt 0 ]; do
	...
	--no-luci)
		...
		;;
	--dongle-modem)
		packages="$packages bsbf-autoconf-dhcp kmod-usb-net-cdc-ether usb-modeswitch"
		shift
		;;
	--mbim-modem)
		packages="$packages bsbf-autoconf-cellular bsbf-quectel-usbnet kmod-usb-serial-option umbim"
		shift
		;;
	--usb-adapters-and-android-tethering)
		packages="$packages bsbf-autoconf-dhcp kmod-usb-net-cdc-ether kmod-usb-net-rtl8152 kmod-usb-net-rndis"
		shift
		;;
	--ios-tethering)
		packages="$packages bsbf-autoconf-dhcp kmod-usb-net-ipheth usbmuxd"
		shift
		;;
	--mikrotik-tools)
